...
'''
MC Basic is a model-free method. Compared to value iteration and policy iteration, data is essential for MC.
'''
class MC_Basic:
    def __init__(self, env = grid_env.GridEnv):
        ...
        self.state_value = np.zeros(shape=self.state_space_size) # 1D list
        print("self.state_value:",self.state_value)
        self.qvalue = np.zeros(shape=(self.state_space_size, self.action_space_size))  # 2D: number of states x number of actions
        self.mean_policy = np.ones(shape=(self.state_space_size, self.action_space_size)) / self.action_space_size  # Average policy, i.e., equal probability for each action
        ...
        print('----------------------------------------------------------------')
    '''
        Define functions required for visualizing the grid world:
    ...
    def obtain_episode(self, policy, start_state, start_action, length):
        """
        :param policy: Generate an episode from the specified policy
        :param start_state: Starting state
        :param start_action: Starting action
        :param length: Maximum length of an episode
        :return: A list of dictionaries in the format: state, action, reward, next_state, next_action
        """
        ...
        while length > 0:
            ...
            episode.append({"state": state, "action": action, "reward": reward, "next_state": next_state,
                            "next_action": next_action})  # Add a dictionary to the list
            if done:
                break
        return episode  # Return the list where elements are dictionaries


    """
    The author jkrose's code here is not the MC_basic code, but the MC_exploring_start every-visit version.
    For the MC_Basic algorithm, please refer to the mc_basic_simple() function.
    """
    def mc_basic(self, length=50, epochs=10):
        """
        :param length: Length of each state-action pair
        :return:
        """
        for epoch in range(epochs):
            for state in range(self.state_space_size):
                for action in range(self.action_space_size):
                    ...
                    # Here the original author used recursion to calculate qvalue. In fact, it can be solved naively.
    ...
    """
    The mc_basic_simple function is consistent with the pseudo-code for the MC_basic algorithm in the book.  2024.5.15
    return: self.state_value To render the result graph, i.e., visualization
    """
    def mc_basic_simple(self, length=20, epochs=10):
        ...
        for epoch in range(epochs):
            for state in range(self.state_space_size):
                """
                Page 97: The first strategy is, in the policy evaluation step, 
                ...
                My understanding: for a state, policy improvement is performed after obtaining the episodes for each action in that state.
                """
                for action in range(self.action_space_size):
                    # Collect sufficiently many episodes starting from (s, a) by following πk
                    for tmp in range(num_episode): # Collect 10 episodes for each action
...
if __name__ == "__main__":
    ...
    for i in episode_length:
        ...
        solver.mc_basic_simple_GUI(length=i, epochs=10)  # Modified to use tqdm for displaying epoch progress
...
if __name__ == "__main1__":
    ...
    for i in tqdm(range(100)):
        time.sleep(0.1)  # Simulate some time-consuming operations
    ...
    # Display time information in the tqdm progress bar description
